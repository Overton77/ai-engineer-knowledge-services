...
export function requireEveReportProofEnvironment(environment: Readonly<Record<string, string | undefined>> = process.env): { readonly database: string; readonly gatewayApiKey: string; readonly inputPath: string; readonly deadlineMs: number } {
  ...
  assert.match(database, /^verification_eve_report_[0-9a-f]{32}$/u, "EVE_REPORT_PROOF_DATABASE_REQUIRED");
  const gatewayApiKey = environment.AI_GATEWAY_API_KEY?.trim() ?? "";
  assert.ok(gatewayApiKey.length > 0, "EVE_REPORT_PROOF_GATEWAY_KEY_REQUIRED");
  const inputPath = environment.VERIFICATION_EVE_REPORT_INPUT?.trim() ?? "";
  assert.ok(inputPath && resolve(inputPath) === inputPath, "EVE_REPORT_INPUT_PATH_REQUIRED");
  const deadlineValue = environment.VERIFICATION_EVE_REPORT_DEADLINE_MS?.trim() ?? "";
  assert.match(deadlineValue, /^\d{13}$/u, "EVE_REPORT_DEADLINE_REQUIRED");
  const deadlineMs = Number(deadlineValue);
  assert.ok(Number.isSafeInteger(deadlineMs) && deadlineMs > Date.now(), "EVE_REPORT_DEADLINE_EXPIRED");
...
function parseEveToolReceipt(stdout: string, expectedOperationId: string): Record<string, unknown> {
  const line = stdout.split(/\r?\n/u).find(value => value.startsWith("EVE_REPORT_TOOL_RECEIPT="));
  assert.ok(line, "EVE_REPORT_TOOL_RECEIPT_MISSING");
  const receipt = JSON.parse(line.slice("EVE_REPORT_TOOL_RECEIPT=".length)) as Record<string, unknown>;
  assert.equal(receipt.status, "succeeded", "EVE_REPORT_TOOL_RECEIPT_STATUS");
  assert.equal(receipt.operationId, expectedOperationId, "EVE_REPORT_TOOL_RECEIPT_OPERATION");
  assert.ok(Object.keys(receipt).every(key => ["status", "operationId", "host", "intent", "tenantId", "requestDigest", "sealedRun", "disposition"].includes(key)), "EVE_REPORT_TOOL_RECEIPT_KEYS");
  return receipt;
}
...
async function main(): Promise<void> {
  ...
  assert.ok(["localhost", "127.0.0.1"].includes(dbUrl.hostname) && dbUrl.port === "54322", "EVE_REPORT_PROOF_LOCAL_DB_ONLY");
  const apiUrl = new URL(local.API_URL); assert.ok(["localhost", "127.0.0.1"].includes(apiUrl.hostname) && apiUrl.port === "54321", "EVE_REPORT_PROOF_LOCAL_STORAGE_ONLY");
  ...
  try {
    ...
    assert.equal(artifactType.length, 1, "EVE_REPORT_RUNTIME_PRINCIPAL_ARTIFACT_TYPE_REQUIRED");
    assert.equal(artifactType[0].description, "Canonical server-resolved runtime principal binding retained for claims and report audit replay", "EVE_REPORT_RUNTIME_PRINCIPAL_ARTIFACT_TYPE_DRIFT");
    ...
    assert.ok(benchmarkCase.assertion.length + benchmarkCase.evidence[0]!.excerpt.length <= 2_000, "EVE_REPORT_D013_CONTENT_BOUND");
    assert.ok(maximumCompletionTokens <= 900, "EVE_REPORT_D013_COMPLETION_TOKEN_BOUND");
    ...
    const assertion = stored.assertions?.[0]?.assertion; assert.ok(assertion, "EVE_REPORT_ASSERTION_REQUIRED");
    ...
    const preflight = prepareGatewaySemanticRequest({ ...blinded, inputArtifactDigest: digestCanonicalJson(blinded) }, model); assert.ok(preflight.requestBytes.byteLength <= 10_000, "EVE_REPORT_D013_WIRE_BOUND");
    const plans: SemanticMissionPlan[] = [{ kind: "report", context, request: fixture.request as VerifyReportRequest, expectedOperationId, workflowId: eveRuntimeLabel, profileArtifact, expectedSemanticRequestDigest: preflight.requestDigest }];    await writeFile(startupPath.replace(/\.json$/u, ".prepared.json"), JSON.stringify({ schemaVersion: "verification-eve-report-prepared.v1", namespace, startupJournal: startupPath, tenantId, proofDatabase: proof.database, frozenCaptureId: frozen.captureId, plannedOperations: plans.map(plan => ({ kind: plan.kind, operationId: plan.expectedOperationId, requestDigest: digestCanonicalJson(plan.request) })), createdAt: new Date().toISOString() }, null, 2) + "\n", { flag: "wx" });
    ...
    assert.ok(claimsReportReads, "EVE_REPORT_TYPED_TERMINAL_READS_REQUIRED");
    ...
    assert.ok(verificationReads, "EVE_REPORT_RUN_READS_REQUIRED");
    ...
    assert.equal(Number(runnableBeforeWorker), 0, "EVE_REPORT_PREEXISTING_RUNNABLE_STEP");
    assert.ok(remainingDeadlineMs() >= 150_000, "EVE_REPORT_DEADLINE_INSUFFICIENT_BEFORE_WORKER");
    ...
    phase = "eve";
    const eveRoot = resolve("../research_ingestion_systems_agent/agents/verification"), fixtureRoot = join(dirname(eveRoot), `.verification-eve-report-${namespace}`);
    ...
    try {
      ...
      await writeFile(resolve(fixtureRoot, "evals/native-eve-report.eval.ts"), `import {defineEval} from "eve/evals"; import {equals} from "eve/evals/expect"; export default defineEval({description:"Real KS Eve report verification; mock model invokes one authored tool",async test(t){const turn=await t.send("Submit the configured report verification.");t.succeeded();turn.calledTool("verify_evidence_bundle",{count:1});const message=turn.events.filter(e=>e.type==="message.completed").at(-1)?.data?.message??turn.data;const value=typeof message==="string"?JSON.parse(message):message;t.check(value?.status,equals("succeeded"));t.check(value?.operationId,equals(process.env.EVE_VERIFICATION_FIXTURE_OPERATION_ID));console.log("EVE_REPORT_TOOL_RECEIPT="+JSON.stringify({status:value?.status,operationId:value?.operationId,host:value?.host,intent:value?.intent,tenantId:value?.tenantId,requestDigest:value?.requestDigest,sealedRun:value?.sealedRun,disposition:value?.disposition}));}});\n`, { flag: "wx" });
      ...
      for (const args of [[eveCli, "build"], [eveCli, "eval", "native-eve-report", "--strict", "--json", "--skip-report"]] as const) {
        ...
        assert.equal(result.exitCode, 0, `EVE_REPORT_${args[1].toUpperCase()}_FAILED`);
        if (args[1] === "build") {
          const raw = await readFile(resolve(fixtureRoot, ".eve", "agent-summary.json"), "utf8");
          assert.ok(Buffer.byteLength(raw, "utf8") <= 64_000, "EVE_REPORT_AGENT_SUMMARY_TOO_LARGE");
          ...
          assert.deepEqual(tools.map(value => (value as { name?: unknown }).name), ["verify_evidence_bundle"], "EVE_REPORT_AGENT_SUMMARY_TOOLS");
          assert.ok(summary.skills === null || (Array.isArray(summary.skills) && summary.skills.length === 0), "EVE_REPORT_AGENT_SUMMARY_SKILLS");
          assert.ok(summary.connections === null || (Array.isArray(summary.connections) && summary.connections.length === 0), "EVE_REPORT_AGENT_SUMMARY_CONNECTIONS");
      ...
      if (fixtureCreated) { const safe = resolve(fixtureRoot); assert.equal(dirname(safe), resolve(dirname(eveRoot)), "EVE_REPORT_FIXTURE_PARENT_INVALID"); assert.match(safe.split(/[\\/]/).at(-1) ?? "", /^\.verification-eve-report-[0-9a-f-]{36}$/u, "EVE_REPORT_FIXTURE_PATH_INVALID"); await rm(safe, { recursive: true, force: true }); }
    }
    ...
    const eveAttestation = await database.transaction(tenantId, async client => {
      ...
      assert.equal(bindings.rows.length, 1, "EVE_REPORT_BINDING_REQUIRED");
      assert.equal(invocations.rows.length, 1, "EVE_REPORT_INVOCATION_REQUIRED");
      const binding = bindings.rows[0]!, invocation = invocations.rows[0]!;
      assert.equal(binding.issuer, eveAuthority.issuer, "EVE_REPORT_BINDING_ISSUER");
      assert.equal(binding.original_key_id, eveKeyId, "EVE_REPORT_BINDING_KEY");
      assert.equal(invocation.issuer, eveAuthority.issuer, "EVE_REPORT_INVOCATION_ISSUER");
      assert.equal(invocation.key_id, eveKeyId, "EVE_REPORT_INVOCATION_KEY");
      assert.equal(invocation.jti, binding.original_jti, "EVE_REPORT_INVOCATION_JTI");
      assert.equal(invocation.invocation_kind, "original", "EVE_REPORT_INVOCATION_KIND");
      assert.match(invocation.envelope_sha256, /^[0-9a-f]{64}$/u, "EVE_REPORT_INVOCATION_ENVELOPE_DIGEST");
      assert.equal((binding.original_external_execution as { runId?: unknown }).runId, (invocation.observed_external_execution as { runId?: unknown }).runId, "EVE_REPORT_EXTERNAL_EXECUTION_BINDING");
    ...
    const typedTerminals = await Promise.all(plans.map(async plan => {
      ...
      assert.equal(response.status, 200, "EVE_REPORT_TYPED_TERMINAL_HTTP_STATUS");
      const http = await response.json(); assert.equal(canonicalizeJson(http), canonicalizeJson(direct), "EVE_REPORT_TYPED_TERMINAL_HTTP_DRIFT");
      assert.equal(direct.operationId, plan.expectedOperationId, "EVE_REPORT_TYPED_TERMINAL_OPERATION_ID");
      assert.equal(direct.tenantId, tenantId, "EVE_REPORT_TYPED_TERMINAL_TENANT");
      assert.equal(direct.requestDigest, digestCanonicalJson(plan.request), "EVE_REPORT_TYPED_TERMINAL_REQUEST_DIGEST");
      const operation = await database.getOperationRecord(tenantId, plan.expectedOperationId); assert.ok(operation, "EVE_REPORT_TYPED_TERMINAL_OPERATION_REQUIRED");
      assert.equal(operation.ownershipMode, "eve", "EVE_REPORT_TYPED_TERMINAL_EVE_OWNERSHIP");
      assert.equal(operation.externalRunId, (eveAttestation.binding.original_external_execution as { runId?: unknown }).runId, "EVE_REPORT_TYPED_TERMINAL_EVE_RUN_REQUIRED");
      assert.ok(["review", "fail", "abstain"].includes(direct.sealedRun.policyOutcome), "EVE_REPORT_TYPED_TERMINAL_POLICY_OUTCOME");
      assert.equal(eveToolReceipt?.disposition, direct.sealedRun.policyOutcome, "EVE_REPORT_TOOL_DISPOSITION_DRIFT");
      assert.equal(canonicalizeJson(eveToolReceipt?.sealedRun), canonicalizeJson({ runId: direct.sealedRun.runId, manifestArtifact: direct.sealedRun.manifestArtifact }), "EVE_REPORT_TOOL_SEALED_RUN_DRIFT");
    ...
    const providerCustody = await database.transaction(tenantId, async client => {
      ...
      assert.deepEqual([...new Set(newlyCreated.rows.map(row => row.operation_id))].sort(), results.map(result => result.operationId).sort(), "EVE_REPORT_PROVIDER_OPERATION_SCOPE");
      return Promise.all(results.map(async result => {
        ...
        const plan = plans.find(item => item.expectedOperationId === result.operationId); assert.ok(plan, "EVE_REPORT_PROVIDER_PLAN_REQUIRED");
        assert.equal(attempts.rows.length, 1, "EVE_REPORT_PROVIDER_ATTEMPT_CARDINALITY");
        const attempt = attempts.rows[0]!;
        assert.notEqual(attempt.actual_cost_micros, null, "EVE_REPORT_PROVIDER_COST_MISSING");
        const actualCostMicros = Number(attempt.actual_cost_micros);
        assert.equal(attempt.state, "settled", "EVE_REPORT_PROVIDER_ATTEMPT_STATE");
        assert.ok(Number.isSafeInteger(actualCostMicros) && actualCostMicros >= 0 && actualCostMicros <= 5_000, "EVE_REPORT_PROVIDER_COST_BOUND");
        assert.equal(`sha256:${attempt.request_sha256}`, plan.expectedSemanticRequestDigest, "EVE_REPORT_PROVIDER_REQUEST_DIGEST_DRIFT");
        ...
        assert.equal(value.observations, 1, "EVE_REPORT_PROVIDER_OBSERVATION_CARDINALITY");
        assert.equal(value.responseCaptures, 1, "EVE_REPORT_PROVIDER_CAPTURE_CARDINALITY");
        return value;
      }));
    });
    ...
    await writeFile(failurePath, JSON.stringify({ schemaVersion: "verification-eve-report-failure.v2", namespace, phase, proofDatabase: proof.database, createdOperationIds, ...diagnostic }, null, 2) + "\n", { flag: "wx" }).catch(() => undefined);
