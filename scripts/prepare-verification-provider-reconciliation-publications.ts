...
try{
 for(const selected of fixture.profiles){
  ...
  const producerProfile=await register({...profile,budget:{...profile.budget,budgetId:reconciliationBudgetId,budgetKey:`reconcile-public-${namespace}`,ceilingCostMicros:400},extractionSchema,profileVersion:`worker-${namespace}`},"verification_structured_extraction_profile");
  ...
  for(const scenario of ["accepted","http"] as const){
   let operationId="";
