...
export function requireCursorReportProofEnvironment(environment: Readonly<Record<string, string | undefined>> = process.env): { readonly database: string; readonly gatewayApiKey: string; readonly deadlineMs: number } {
  const database = environment.VERIFICATION_PROOF_DATABASE?.trim() ?? "";
  assert.match(database, /^verification_cursor_report_[0-9a-f]{32}$/u, "CURSOR_REPORT_PROOF_DATABASE_REQUIRED");
  const gatewayApiKey = environment.AI_GATEWAY_API_KEY?.trim() ?? "";
  assert.ok(gatewayApiKey.length > 0, "CURSOR_REPORT_PROOF_GATEWAY_KEY_REQUIRED");
  const deadlineValue = environment.VERIFICATION_CURSOR_REPORT_DEADLINE_MS?.trim() ?? '';
  assert.match(deadlineValue, /^\d{13}$/u, 'CURSOR_REPORT_DEADLINE_REQUIRED');
  const deadlineMs = Number(deadlineValue);
  assert.ok(Number.isSafeInteger(deadlineMs) && deadlineMs > Date.now(), 'CURSOR_REPORT_DEADLINE_EXPIRED');
  return { database, gatewayApiKey, deadlineMs };
}
...
async function main(): Promise<void> {
  const proof = requireCursorReportProofEnvironment();
  ...
  assert.ok(["localhost", "127.0.0.1"].includes(dbUrl.hostname) && dbUrl.port === "54322", "CURSOR_REPORT_PROOF_LOCAL_DB_ONLY");
  const apiUrl = new URL(local.API_URL); assert.ok(["localhost", "127.0.0.1"].includes(apiUrl.hostname) && apiUrl.port === "54321", "CURSOR_REPORT_PROOF_LOCAL_STORAGE_ONLY");
  const temporalAddress = process.argv[2]?.trim() || "127.0.0.1:7233";
  assert.match(temporalAddress, /^(127\.0\.0\.1|localhost):\d{1,5}$/u, "CURSOR_REPORT_PROOF_LOCAL_TEMPORAL_ONLY");
  ...
  const inputPath = process.env.VERIFICATION_CURSOR_REPORT_INPUT?.trim() ?? "";
  assert.ok(inputPath && resolve(inputPath) === inputPath, "CURSOR_REPORT_INPUT_PATH_REQUIRED");
  const cursor = JSON.parse(await readFile(inputPath, "utf8")) as CursorInput;
  assert.ok(typeof cursor.reportMarkdown === "string" && cursor.reportMarkdown.length > 0 && cursor.reportMarkdown.length <= 3_000 && typeof cursor.producerAgentId === "string" && cursor.producerAgentId.length > 0 && cursor.producerAgentId.length <= 200 && typeof cursor.producerRunId === "string" && cursor.producerRunId.length > 0 && cursor.producerRunId.length <= 200 && cursor.assertion && typeof cursor.assertion.text === "string", "CURSOR_REPORT_INPUT_INVALID");
  const provider = await loadDiagnosticsV1ProviderGrant(resolve("catalog/verification-benchmarks/diagnostics-companies-v1"));
  const benchmarkCase = provider.dataset.cases.find(item => item.caseId === "tru-symphony-source"); assert.ok(benchmarkCase, "CURSOR_REPORT_V1_CASE_REQUIRED");
  const frozen = registry.records.find(record => record.sourceKey === benchmarkCase.evidence[0]!.sourceKey); assert.ok(frozen && frozen.projections.length > 0, "CURSOR_REPORT_PROOF_FROZEN_PROJECTION_REQUIRED");
  const startupPath = resolve("../internal", `verification-cursor-report-startup-${namespace}.json`), outputPath = resolve("../internal", `verification-cursor-report-${namespace}.json`), failurePath = outputPath.replace(/\.json$/u, ".failure.json");
  await writeFile(startupPath, JSON.stringify({ schemaVersion: "verification-cursor-report-startup.v1", namespace, tenantId: registry.tenantId, proofDatabase: proof.database, temporalAddress, plannedKinds: ["report"], intendedMutationScope: "isolated local mission, attempts, semantic claims/report operations and provider evidence", createdAt: new Date().toISOString() }, null, 2) + "\n", { flag: "wx" });
  ...
  const failureDiagnostic = async (error: unknown) => {
    const errorCode = error instanceof Error && /^[A-Z0-9_]{3,160}$/u.test(error.message) ? error.message : error instanceof Error ? error.name : "UNKNOWN";
  ...
  try {
    ...
    assert.equal(artifactType.length, 1, "CURSOR_REPORT_RUNTIME_PRINCIPAL_ARTIFACT_TYPE_REQUIRED");
    assert.equal(artifactType[0].description, "Canonical server-resolved runtime principal binding retained for claims and report audit replay", "CURSOR_REPORT_RUNTIME_PRINCIPAL_ARTIFACT_TYPE_DRIFT");
    ...
    const admission = new VerificationAdmissionService(repository, { async parse(): Promise<never> { throw new Error("PARSER_FORBIDDEN_IN_CURSOR_REPORT_MISSION_PROOF"); } }, { parserVersion: projection.parserVersion as "verification-native-parser.v1", imageDigest: projection.imageDigest, limits: VERIFICATION_PARSER_LIMITS }, { storageBucket: bucket, producerVersion: "cursor-report.v1", encryptionClass: "supabase-managed", retentionClass: "verification-audit", now: () => new Date().toISOString() });
    const tenantId = registry.tenantId, missionId = randomUUID(), producerWorkItemId = randomUUID(), producerAttemptId = randomUUID(), producerDeploymentId = cursor.producerAgentId, verifierDeploymentId = `cursor-report-verifier-${namespace}`;
    const actor: Actor = { kind: "service", id: randomUUID(), serviceIdentity: "mission_control_client" };
    await database.transaction(tenantId, async client => { await client.query("insert into orchestration.mission(id,tenant_id,slug,goal) values($1,$2,$3,$4)", [missionId, tenantId, `cursor-report-${namespace}`, "Bounded Mission Control semantic claims/report proof"]); await client.query("insert into orchestration.work_item(id,tenant_id,mission_id,kind,spec) values($1,$2,$3,'verify_extraction',$4::jsonb)", [producerWorkItemId, tenantId, missionId, JSON.stringify({ namespace, role: "semantic_mission_fixture" })]); await client.query("insert into orchestration.attempt(id,tenant_id,work_item_id,attempt_no,agent_deployment_id,started_at) values($1,$2,$3,1,$4,clock_timestamp())", [producerAttemptId, tenantId, producerWorkItemId, producerDeploymentId]); });
    const register = async (value: unknown, artifactType: string, label: string): Promise<VerificationArtifactHandle> => repository.registerContentAddressedArtifact({ tenantId, producerAttemptId, missionId, artifactType, bytes: new TextEncoder().encode(canonicalizeJson(value)), createdAt: new Date().toISOString(), parentArtifactIds: [], producerActivityId: "verification-cursor-report", producerVersion: "v1", mediaType: "application/json", encryptionClass: "supabase-managed", retentionClass: "verification-audit", dataClassification: "restricted", bucketClass: "ledger", storageBucket: bucket });
    const policyVersion = `cursor-report-policy-${namespace}`, policy: VerificationPolicyDefinition = { schemaVersion: "verification-policy.v1", policyVersion, definitionId: `cursor-report-${namespace}`, criticalDownstreamUses: [], requireCrossFamilyForRisk: [], requireIndependentAuthorityForScopes: [], mixedEvidenceOutcome: "review", unknownCriticalOutcome: "review", authorityWithheldOutcome: "review", reviewAvailable: true };
    const policyArtifact = await register(policy, "verification_policy", "policy");
    const key = generateKeyPairSync("ed25519"), keyId = `cursor-report-${namespace}`, privateKeyPem = key.privateKey.export({ type: "pkcs8", format: "pem" }).toString(), publicKeyPem = key.publicKey.export({ type: "spki", format: "pem" }).toString();
    const maximumCompletionTokens = 900;
    assert.ok(benchmarkCase.assertion.length + benchmarkCase.evidence[0]!.excerpt.length <= 2_000, "CURSOR_REPORT_D013_CONTENT_BOUND");
    assert.ok(maximumCompletionTokens <= 900, "CURSOR_REPORT_D013_COMPLETION_TOKEN_BOUND");
    const workItemId = randomUUID(), attemptId = randomUUID(), operation = "verifyReport" as const, expectedOperationId = semanticMissionOperationId({ tenantId, operation, missionId, workItemId, attemptId }), workflowId = `cursor-report-${namespace}`;
    await database.transaction(tenantId, async client => { await client.query("insert into orchestration.work_item(id,tenant_id,mission_id,kind,spec) values($1,$2,$3,'verify_extraction',$4::jsonb)", [workItemId, tenantId, missionId, JSON.stringify({ namespace, role: "cursor_agent_report", producerAgentId: cursor.producerAgentId, producerRunId: cursor.producerRunId })]); await client.query("insert into orchestration.attempt(id,tenant_id,work_item_id,attempt_no,agent_deployment_id,started_at) values($1,$2,$3,1,$4,clock_timestamp())", [attemptId, tenantId, workItemId, verifierDeploymentId]); });
    ...
    const context: OperationContext = { contractVersion: "v1", tenantId, operationId: expectedOperationId, attemptId, missionId, workItemId, correlationId: `cursor-report-${namespace}`, actor: {...actor,id:randomUUID()}, capabilityVersion: "verification.v1", idempotencyKey: ["vd1", operation, tenantId, missionId, workItemId, attemptId].join(":"), reason: "Authorized bounded Cursor-authored report verification", externalExecution: { runtime: "mission_control", runId: workflowId } };
    ...
    const assertion = stored.assertions?.[0]?.assertion; assert.ok(assertion, "CURSOR_REPORT_ASSERTION_REQUIRED");
    ...
    const preflight = prepareGatewaySemanticRequest({ ...blinded, inputArtifactDigest: digestCanonicalJson(blinded) }, model); assert.ok(preflight.requestBytes.byteLength <= 10_000, "CURSOR_REPORT_D013_WIRE_BOUND");
    const plans: SemanticMissionPlan[] = [{ kind: "report", context, request: fixture.request as VerifyReportRequest, expectedOperationId, workflowId, profileArtifact, expectedSemanticRequestDigest: preflight.requestDigest }];    await writeFile(startupPath.replace(/\.json$/u, ".prepared.json"), JSON.stringify({ schemaVersion: "verification-cursor-report-prepared.v1", namespace, startupJournal: startupPath, tenantId, proofDatabase: proof.database, temporalAddress, frozenCaptureId: frozen.captureId, plannedOperations: plans.map(plan => ({ kind: plan.kind, operationId: plan.expectedOperationId, workflowId: plan.workflowId, requestDigest: digestCanonicalJson(plan.request) })), createdAt: new Date().toISOString() }, null, 2) + "\n", { flag: "wx" });
    ...
    assert.ok(claimsReportReads, "CURSOR_REPORT_TYPED_TERMINAL_READS_REQUIRED");
    ...
    assert.ok(verificationReads, "CURSOR_REPORT_RUN_READS_REQUIRED");
    ...
    const token = `cursor-report-token-${namespace}`;
    ...
    assert.ok(remainingDeadlineMs() >= 60_000, 'CURSOR_REPORT_DEADLINE_INSUFFICIENT_BEFORE_WORKER');
    ...
    const typedTerminals = await Promise.all(plans.map(async plan => {
      ...
      assert.equal(response.status, 200, "CURSOR_REPORT_TYPED_TERMINAL_HTTP_STATUS");
      const http = await response.json(); assert.equal(canonicalizeJson(http), canonicalizeJson(direct), "CURSOR_REPORT_TYPED_TERMINAL_HTTP_DRIFT");
      const operation = await database.getOperationRecord(tenantId, plan.expectedOperationId); assert.ok(operation, "CURSOR_REPORT_TYPED_TERMINAL_OPERATION_REQUIRED");
      assert.equal(operation.ownershipMode, "mission_control", "CURSOR_REPORT_TYPED_TERMINAL_OWNERSHIP"); assert.equal(operation.externalRunId, plan.workflowId, "CURSOR_REPORT_TYPED_TERMINAL_EXTERNAL_RUN");
    ...
    const providerCustody = await database.transaction(tenantId, async client => Promise.all(results.map(async result => {
      ...
      const plan = plans.find(item => item.expectedOperationId === result.operationId); assert.ok(plan, "CURSOR_REPORT_PROVIDER_PLAN_REQUIRED");
      ...
      assert.ok(value.providerAttempts >= 1 && value.observations >= 1 && value.responseCaptures >= 1, "CURSOR_REPORT_PROVIDER_CUSTODY_REQUIRED");
      assert.ok(attempts.rows.some(row => `sha256:${row.request_sha256}` === plan.expectedSemanticRequestDigest), "CURSOR_REPORT_PROVIDER_REQUEST_DIGEST_DRIFT");
      return value;
    })));
    ...
    await writeFile(failurePath, JSON.stringify({ schemaVersion: "verification-cursor-report-failure.v2", namespace, phase, proofDatabase: proof.database, createdOperationIds, ...diagnostic }, null, 2) + "\n", { flag: "wx" }).catch(() => undefined);
  ...
  finally { if (worker) await worker.stop("cursor-report-proof"); if (server) await server.close(); await database.close(); }
}
