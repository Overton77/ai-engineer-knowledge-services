...
try{
 for(const selected of fixture.profiles){
  ...
  for(const scenario of ["accepted","http","schema"] as const){
   ...
   checks[`${selected.providerId}_${scenario}_actual_child_sigkill_before_terminal`]=true;
 ...
 const report={schemaVersion:"verification-structured-extraction-process-recovery-proof.v1",namespace,tenantId,fixtureName,createdAt:new Date().toISOString(),syntheticFetches,supplierRequests:0,checks,publicKeyPem,dirtyStateArtifact,sourceFiles:sourceFiles.map(({bytesBase64,...file})=>file),results,limitations:["Injected synthetic transport only; no supplier billing verification","Six actual child SIGKILL events after committed publication and before terminal receipt; other crash gaps remain pending","Actual loopback public submission/read with explicit synthetic buildServer admission; production API bootstrap requires live_provider mode","Source custody covers listed files only"]};
 const output=resolve("../internal",`verification-structured-extraction-process-recovery-${namespace}.json`);await writeFile(output,JSON.stringify(report,null,2)+"\n");console.log(JSON.stringify({output,checks:Object.keys(checks).length,syntheticFetches}));
