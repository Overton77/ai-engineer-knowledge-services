...
const reconciliationBudgetId=randomUUID();
...
try{
 for(const selected of fixture.profiles){
  ...
  const producerProfile=await register({...profile,budget:{...profile.budget,budgetId:reconciliationBudgetId,budgetKey:`reconcile-${namespace}`,ceilingCostMicros:200},extractionSchema,profileVersion:`worker-${namespace}`},"verification_structured_extraction_profile");
  ...
  for(const scenario of ["accepted"] as const){
   ...
   const cancelled=selected.providerId==="interfaze-extraction.v1";
   if(cancelled)await database.cancelOperation(tenantId,operationId,{actorIdentity:namespace,correlationId:namespace});else await assert.rejects(worker.runOperationOnce(operationId),/Original dispatch has no captured response/);
   assert.equal(syntheticFetches,before+1);assert.equal((await database.getOperation(tenantId,operationId))?.status,cancelled?"cancelled":"failed");
   ...
   assert.equal(provider.length,1);assert.equal(provider[0]!.state,cancelled?"dispatched":"uncertain");assert.equal(provider[0]!.actual_cost_micros,null);assert.equal(provider[0]!.response_artifact_id,null);assert.equal(Number(provider[0]!.reservation_cost_micros),100);
   const receipts=await database.listReceipts(tenantId,operationId);assert.equal(receipts.length,cancelled?0:1);if(!cancelled)assert.equal(receipts[0]!.receiptKind,"failure");
