...
describe("claims/report read runtime",()=>{
  ...
  it("fails closed when signature trust is configured without native storage and ownership",()=>{
    ...
    const environment={VERIFICATION_CLAIMS_REPORT_READ_PUBLIC_KEYS_JSON:JSON.stringify([{keyId:"trusted",publicKeyPem}])};
    expect(()=>createVerificationClaimsReportReads(undefined,environment)).toThrow("VERIFICATION_CLAIMS_REPORT_READS_STORAGE_AND_OWNERSHIP_REQUIRED");
  });
});
