...
const headers = {
  authorization: "Bearer semantic-route-test",
  "x-tenant-id": tenantId,
  "x-correlation-id": "semantic-route",
  "idempotency-key": "semantic-route-key",
};
...
const resource = {
  ...
  operationId,
  providerAttemptId,
  artifact: { artifactId: artifact.artifactId, digest },
  actualCostMicros: 0,
  releasedReservationCostMicros: 1,
  appliedAt: "2026-09-07T00:30:00.000Z",
  redispatchAuthorized: false as const,
};
const resolveIdentity = () => ({
  actor,
  grants: [{ tenantId, roles: ["knowledge_operator" as const], scopes: [] }],
});
...
describe("semantic claims/report reconciliation routes", () => {
  it("keeps semantic service host-scoped and validates its compact result", async () => {
    const getDecision = vi.fn(async (input) => {
    ...
    const applyDecision = vi.fn(async (input) => {
      expect(input).toMatchObject({
        tenantId,
        operationId,
        providerAttemptId,
    ...
    try {
      for (const path of ["claims", "reports"])
        expect(
          (
            await api.inject({
              method: "GET",
              url: `/v1/verification/${path}/${operationId}/provider-attempts/${providerAttemptId}/reconciliation`,
              headers,
            })
          ).statusCode,
        ).toBe(200);
      ...
      expect(posted.json()).toEqual(resource);
      const malformed = await api.inject({
        method: "POST",
        url: `/v1/verification/claims/${operationId}/provider-attempts/${providerAttemptId}/reconciliation`,
        headers,
        payload: { artifact, context: { actor: { kind: "human", id: id(9) } } },
      });
      expect(malformed.statusCode).toBe(400);
      expect(applyDecision).toHaveBeenCalledOnce();
    } finally {
      await api.close();
    }
  });
  ...
  it("rejects a schema-valid reconciliation result outside the route scope", async () => {
    const api = buildServer({
      resolveIdentity,
      verificationSemanticReconciliation: {
        getDecision: async () => ({ ...resource, providerAttemptId: id(99) }),
        applyDecision: async () => resource,
      } as never,
    });
    try {
  ...
  it("returns capability unavailable without changing extraction reconciliation behavior", async () => {
    ...
    try {
      const semantic = await api.inject({
        method: "GET",
        url: `/v1/verification/claims/${operationId}/provider-attempts/${providerAttemptId}/reconciliation`,
        headers,
      });
      const extraction = await api.inject({
        method: "GET",
      ...
      expect(extraction.json()).toMatchObject({
        code: "CAPABILITY_NOT_ADMITTED",
      });
    } finally {
      await api.close();
