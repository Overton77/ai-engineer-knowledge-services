  ...
  VERIFICATION_SERVICE_OWNERSHIP_GRANTS_JSON: JSON.stringify([
    {
      ...
      capabilityVersion: "verification-semantic.v1",
    },
  ]),
    ...
    expect(() =>
      createVerificationSemanticReconciliationService(undefined, {
        VERIFICATION_SEMANTIC_PROVIDER_RECONCILIATION_GRANTS_JSON:
          environment.VERIFICATION_SEMANTIC_PROVIDER_RECONCILIATION_GRANTS_JSON,
      }),
    ).toThrow("SEMANTIC_RECONCILIATION_STORAGE_KEYS_OWNERSHIP_REQUIRED");
  });

  it("denies wrong actor, operation, attempt, and host ownership before artifact hydration", async () => {
    const query = vi.fn(async (_sql: string, params?: unknown[]) => ({
      rows: Array.isArray(params?.[2]) ? [] : [],
    }));
    const database = {
    ...
    for (const input of [
      ...
      await expect(service.getDecision(input)).rejects.toMatchObject({
        code: "NOT_FOUND",
      });
