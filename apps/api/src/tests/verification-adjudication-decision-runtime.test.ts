...
describe("decision API runtime configuration", () => {
  ...
  it("requires canonical ownership and signed packet verification", () => {
    ...
    expect(() =>
      create({} as never, {
        ...
        VERIFICATION_SERVICE_OWNERSHIP_GRANTS_JSON: "[]",
      }),
    ).toThrow("SIGNED_READER_REQUIRED");
  });
  it("rejects caller-like provenance, ambiguous and oversized server grants", () => {
    ...
    for (const value of [
      "{",
      JSON.stringify([{ ...grant, provenance: "human_origin" }]),
      JSON.stringify([grant, grant]),
    ])
      expect(() =>
        create({} as never, {
          ...base,
          VERIFICATION_ADJUDICATION_SYNTHETIC_REVIEWER_GRANTS_JSON: value,
        }),
      ).toThrow("GRANTS_INVALID");
    expect(() =>
      create({} as never, {
        ...base,
