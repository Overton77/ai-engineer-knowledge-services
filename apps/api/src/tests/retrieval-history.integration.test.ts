...
} from "../../../../packages/persistence/test/retrieval-history-fixture.mjs";
import {
  createSelectedCandidateFixture,
  type SelectedCandidateFixture,
  type SelectedCandidateFixtureOptions,
...
const CURRENT_QUERY =
  "Synthetic Child and Parent organizations operate together in preview";
...
const HISTORY_ACTOR_ENCODING =
  "Official publication must be authorized by the persisted operation identities";
...
async function admitRetrievalPolicy(
    ...
    async (client) => {
      await client.query(
        ...
        [
          policyId,
          fixture.tenantId,
          `t7-retrieval-${policyId.slice(0, 8)}`,
          "T7 official retrieval policy",
        ],
      );
      await client.query(
        `insert into retrieval.retrieval_policy_version
...
function createRetrievalApi(input: {
  ...
  const readArtifact = createRemoteRetrievalArtifactReader(
    input.fixture.database,
    {
  ...
  return buildServer({
    ...
    getEvidencePacket: async (tenantId, packetId) => {
      const packet = await input.fixture.database.getEvidencePacket(
        tenantId,
        packetId,
      );
    ...
    replayEvidencePacketCitations: (tenantId, packetId) =>
      input.fixture.database.replayEvidencePacketCitations(
        tenantId,
        packetId,
        readArtifact,
      ),
    canonicalRetrievalExecutor: new CanonicalRetrievalExecutor(
      input.fixture.database,
      input.fixture.embeddingAdapter,
    ),
    publicOrigin: "http://127.0.0.1:54431",
    resolveIdentity: (token) => {
      if (token === TOKEN_A)
        return operatorIdentity(input.fixture.tenantId, input.actor);
  ...
  const response = await input.api.inject({
    ...
    payload: {
      context: {
        ...
        correlationId,
        actor: input.actor,
        capabilityVersion: "retrieval/v1",
        idempotencyKey: `t7:${operationId}`,
        reason: "T7 official retrieval",
        contractVersion: "v1",
      },
...
async function readAuthorizedPacket(input: {
  ...
  const run = await input.api.inject({
    method: "GET",
    url: `/v1/retrieval-runs/${input.operationId}`,
    headers: {
      authorization: `Bearer ${input.token}`,
      "x-tenant-id": input.tenantId,
      "x-correlation-id": `read-${input.operationId}`,
    },
  });
  ...
  const packet = await input.api.inject({
    method: "GET",
    url: `/v1/evidence-packets/${packetIds[0]}`,
    headers: {
      authorization: `Bearer ${input.token}`,
      "x-tenant-id": input.tenantId,
      "x-correlation-id": `packet-${packetIds[0]}`,
    },
  });
...
describe.skipIf(!databaseUrl || !storage)(
  "T7 temporal retrieval and citation replay through the API",
  () => {
    let harness: RetrievalHarness;

    beforeAll(async () => {
      const fixture = await createSelectedCandidateFixture({
      ...
      const operationId = await publishCandidate(fixture, {
        ...
        steps: 2,
        reason:
          "Activate the independently evaluated selected candidate for T7 retrieval",
      });
      const publicationId = await publicationIdFor(
        fixture,
        operationId,
        "verify.succeeded",
      );
      const policyVersionId = await admitRetrievalPolicy(fixture);
      const resolved = await fixture.database.resolveRetrievalPolicy(
        fixture.tenantId,
        policyVersionId,
        [PUBLICATION_SPACE],
      );
      if (!resolved.targets.length)
        throw new Error(`T7_NO_ACTIVE_TARGETS:${resolved.version}`);
      const publications = await fixture.database.retrievalPublications(
        fixture.tenantId,
        resolved.targets.map((target) => target.vectorSpaceVersionId),
      );
    ...
    it(
      "returns an authorized current packet with full support, not a bag of ids",
      async () => {
        const posted = await postRetrieval({
          api: harness.api,
          tenantId: harness.fixture.tenantId,
          token: TOKEN_A,
          actor: harness.actor,
          plan: retrievalPlan(harness.policyVersionId, CURRENT_QUERY),
        });
        const failed =
          posted.response.statusCode === 202
            ? undefined
            : await harness.api.inject({
                method: "GET",
                url: `/v1/operations/${posted.operationId}`,
                headers: {
                  authorization: `Bearer ${TOKEN_A}`,
                  "x-tenant-id": harness.fixture.tenantId,
                  "x-correlation-id": `diag-${posted.operationId}`,
                },
              });
        harness.currentOperationId = posted.operationId;
        if (!harness.publicationAuthorized) {
          expect(
            posted.response.statusCode,
            `${HISTORY_ACTOR_ENCODING}; ${posted.response.body}`,
          ).toBe(409);
          throw new Error(
            `T7_CURRENT_PACKET_BLOCKED:${HISTORY_ACTOR_ENCODING}`,
          );
        }
        expect(
          posted.response.statusCode,
          `${posted.response.body} operation=${failed?.body}`,
        ).toBe(202);
        ...
        expect(
          packet.queryClock?.publications?.some(
            (item) => item.publicationId === harness.publicationId,
          ),
        ).toBe(true);
        ...
        for (const member of packet.members) {
          ...
          for (const path of member.support!.paths) {
            ...
            expect(path.captureArtifact.digest).toMatch(
              /^sha256:[0-9a-f]{64}$/,
            );
    ...
    it(
      "omits the preview record when no admitted world-time witness covers the query",
      async () => {
        const posted = await postRetrieval({
          api: harness.api,
          tenantId: harness.fixture.tenantId,
          ...
          actor: harness.actor,
          plan: retrievalPlan(harness.policyVersionId, CURRENT_QUERY, {
            worldScope: { kind: "at", at: PRE_VALIDITY_WORLD },
          }),
        });
        ...
        expect(historical.plan.worldScope).toEqual({
          kind: "at",
          at: PRE_VALIDITY_WORLD,
        });
        expect(historical.queryClock?.worldScope).toEqual({
          kind: "at",
          at: PRE_VALIDITY_WORLD,
        });
    ...
    it(
      "keeps tenant B from reading tenant A's run or packet",
      async () => {
        const runId =
          harness.currentPacket?.retrievalRunId ??
          harness.currentOperationId ??
          (
            await postRetrieval({
              api: harness.api,
              tenantId: harness.fixture.tenantId,
        ...
        const owner = await harness.api.inject({
          method: "GET",
          url: `/v1/operations/${runId}`,
          headers: {
            authorization: `Bearer ${TOKEN_A}`,
            "x-tenant-id": harness.fixture.tenantId,
            "x-correlation-id": `owner-${runId}`,
          },
        });
        ...
        const headers = {
          ...
          "x-correlation-id": `tenant-b-${randomUUID()}`,
        };
        const run = await harness.api.inject({
          method: "GET",
          url: `/v1/operations/${runId}`,
          headers,
        });
        ...
        if (harness.currentPacket) {
          const packet = await harness.api.inject({
            method: "GET",
            url: `/v1/evidence-packets/${harness.currentPacket.id}`,
            headers,
          });
          const citations = await harness.api.inject({
            method: "GET",
            url: `/v1/evidence-packets/${harness.currentPacket.id}/citations`,
            headers,
          });
        ...
        const forged = await harness.api.inject({
          ...
          headers: {
            authorization: `Bearer ${TOKEN_B}`,
            "x-tenant-id": harness.fixture.tenantId,
            "x-correlation-id": `forged-${randomUUID()}`,
          },
        });
    ...
    it(
      "returns RETRIEVAL_CAPABILITY_UNSUPPORTED 422 for a required unimplemented capability",
      async () => {
        const posted = await postRetrieval({
          api: harness.api,
          tenantId: harness.fixture.tenantId,
        ...
        const body = RetrievalUnsupportedResponseSchema.parse(
    ...
    it(
      "replays remote citation bytes after producer local files are gone",
      async () => {
        if (!harness.currentPacket)
          throw new Error(`T7_REPLAY_BLOCKED:${HISTORY_ACTOR_ENCODING}`);
        const current = harness.currentPacket;
        expect(
          current!.members.some(
            (member) => (member.support?.paths.length ?? 0) > 0,
          ),
        ).toBe(true);
        await rm(harness.fixture.producerDirectory, {
          recursive: true,
          force: true,
        });
        ...
        const replayed = await harness.api.inject({
          method: "GET",
          url: `/v1/evidence-packets/${current!.id}/citations`,
          headers: {
            authorization: `Bearer ${TOKEN_A}`,
            "x-tenant-id": harness.fixture.tenantId,
        ...
        for (const citation of body.citations) {
          ...
          expect(citation.selectedSizeBytes).toBe(
            Buffer.byteLength(citation.selectedText, "utf8"),
          );
          expect(citation.selectedSizeBytes).toBeGreaterThan(0);
          expect(citation.captureDigest).toMatch(/^sha256:[0-9a-f]{64}$/);
          expect(citation.selectedContentDigest).toMatch(
            /^sha256:[0-9a-f]{64}$/,
          );
    ...
    it(
      "omits revoked support from a later official current retrieval",
      async () => {
        if (!harness.publicationAuthorized)
          throw new Error(`T7_REVOCATION_BLOCKED:${HISTORY_ACTOR_ENCODING}`);
...
describe.skipIf(!databaseUrl || !storage)(
  ...
  () => {
    ...
    beforeAll(async () => {
      fixture = await buildRetrievalHistoryFixture({
        databaseUrl: databaseUrl!,
        storage: storage!,
      });
    ...
    async function packetAt(
      scope: Pick<RetrievalPlan, "worldScope" | "knowledgeScope">,
    ) {
      const posted = await postRetrieval({
        api,
        tenantId: fixture.tenantId,
        ...
        plan: retrievalPlan(
          policyVersionId,
          "Synthetic Model X API announcement general availability",
          scope,
        ),
      });
      expect(posted.response.statusCode, posted.response.body).toBe(202);
      ...
      if (scope.knowledgeScope)
        expect(packet.queryClock?.atKnowledgeSeq).toBe(
          scope.knowledgeScope.atKnowledgeSeq,
        );
      expect(
        packet.queryClock?.publications?.some(
          (binding) => binding.publicationId === fixture.publicationId,
        ),
      ).toBe(true);
      return packet;
    }
    ...
    function assertClaims(packet: EvidencePacket, keys: string[]) {
      const expected = fixture.temporalClaims.filter((claim) =>
        keys.includes(claim.key),
      );
      expect(
        packet.members
          .map((member) => member.support?.target.canonicalId)
          .sort(),
      ).toEqual(expected.map((claim) => claim.claimId).sort());
      for (const member of packet.members) {
        expect(member.support?.target.kind).toBe("claim");
        expect(member.support?.paths).toHaveLength(1);
        const claim = expected.find(
          (item) => item.claimId === member.support!.target.canonicalId,
        )!;
        expect(member.support!.paths[0]!.claimId).toBe(claim.claimId);
        expect(member.support!.paths[0]!.selectedContentDigest).toBe(
          sha256Digest(claim.statement),
        );
        expect(member.support!.paths[0]!.captureArtifact.digest).toMatch(
          /^sha256:[0-9a-f]{64}$/,
        );
        expect(member.locators.length).toBeGreaterThan(0);
        expect(member.artifactReferences.length).toBeGreaterThan(0);
    ...
    it(
      "returns only the admitted event at each world time, with exact supporting quote",
      async () => {
        for (const event of retrievalHistoryEvents) {
          assertClaims(
            await packetAt({ worldScope: { kind: "at", at: event.from } }),
            [event.key],
          );
          assertClaims(
            await packetAt({ worldScope: { kind: "at", at: event.to! } }),
            [],
          );
        }
        assertClaims(
          await packetAt({
            worldScope: {
              kind: "overlap",
              from: retrievalHistoryEvents[0]!.from,
              to: retrievalHistoryEvents[1]!.from,
            },
          }),
          ["announcement"],
        );
    ...
    it(
      "excludes GA at the earlier knowledge sequence even when world time is GA day",
      async () => {
        const announcement = fixture.temporalClaims.find(
          (claim) => claim.key === "announcement",
        )!;
        ...
        assertClaims(
          await packetAt({
            knowledgeScope,
            worldScope: { kind: "at", at: retrievalHistoryEvents[1]!.from },
          }),
          [],
        );
        assertClaims(
          await packetAt({
            knowledgeScope: { atKnowledgeSeq: ga.knowledgeSeq },
            worldScope: { kind: "at", at: retrievalHistoryEvents[1]!.from },
          }),
          ["ga"],
        );
        assertClaims(
          await packetAt({
            knowledgeScope: { atKnowledgeSeq: announcement.knowledgeSeq - 1 },
          }),
          [],
        );
      },
    ...
    it(
      "replays each temporal packet's exact quote from remote custody after producer teardown",
      async () => {
        const packets = [];
        for (const event of retrievalHistoryEvents) {
          packets.push({
            event,
            packet: await packetAt({
              worldScope: { kind: "at", at: event.from },
            }),
          });
        }
        ...
        for (const { event, packet } of packets) {
          const response = await api.inject({
            method: "GET",
            url: `/v1/evidence-packets/${packet.id}/citations`,
            headers: {
              authorization: `Bearer ${TOKEN_A}`,
              "x-tenant-id": fixture.tenantId,
              "x-correlation-id": `history-replay-${packet.id}`,
            },
          });
          ...
          expect(replay.citations[0]!.selectedContentDigest).toBe(
            sha256Digest(event.statement),
          );
        }
      },
      FIXTURE_TIMEOUT_MS,
...
const scopedAvailability: NonNullable<
  SelectedCandidateFixtureOptions["temporalEvents"]
> = [
  {
    key: "api-announced",
    statement:
      "Synthetic Model X API was announced on January 15 and remained announcement-only until February 1 2026.",
    availability: { scope: "api", status: "announced" },
  ...
  },
  {
    key: "api-preview",
    statement:
      "Synthetic Model X API was in preview from February 1 until June 10 2026.",
    availability: { scope: "api", status: "preview" },
  ...
  },
  {
    key: "api-ga",
    statement:
      "Synthetic Model X API was reported generally available from June 10 2026.",
    availability: { scope: "api", status: "ga" },
  ...
  },
  {
    key: "chatgpt-preview",
    statement:
      "Synthetic Model X in ChatGPT was in preview from February 1 until July 20 2026.",
    availability: { scope: "chatgpt", status: "preview" },
    from: "2026-02-01T00:00:00Z",
  ...
  },
  {
    key: "chatgpt-ga",
    statement:
      "Synthetic Model X in ChatGPT became generally available on July 20 2026.",
    availability: { scope: "chatgpt", status: "ga" },
  ...
  },
  {
    key: "api-correction",
    statement:
      "Correction: Synthetic Model X API remained in preview from June 10 until June 12 2026.",
    availability: { scope: "api", status: "preview" },
    from: "2026-06-10T00:00:00Z",
  ...
  },
  {
    key: "api-challenge",
    statement:
      "A supported challenge disputes Synthetic Model X API general availability after June 12 2026.",
    challengeOf: "api-ga",
...
describe.skipIf(!databaseUrl || !storage)(
  "T08/T09 corrected and surface-scoped temporal packets",
  () => {
    ...
    const claim = (key: string) =>
      fixture.temporalClaims.find((item) => item.key === key)!;

    beforeAll(async () => {
      fixture = await buildRetrievalHistoryFixture({
        databaseUrl: databaseUrl!,
        storage: storage!,
        temporalEvents: scopedAvailability,
      });
    ...
    async function scopedPacket(input: {
      at: string;
      entityId?: string;
      knowledgeSeq?: number;
    }) {
      const plan = retrievalPlan(
        policyVersionId,
        "Synthetic Model X availability",
        {
          worldScope: { kind: "at", at: input.at },
          knowledgeScope: {
            atKnowledgeSeq:
              input.knowledgeSeq ?? claim("api-correction").knowledgeSeq,
          },
        },
      );
    ...
    function expectMembers(packet: EvidencePacket, keys: string[]) {
      expect(
        packet.members
          .map((member) => member.support!.target.canonicalId)
          .sort(),
      ).toEqual(keys.map((key) => claim(key).claimId).sort());
      for (const member of packet.members) {
        ...
        const original = fixture.temporalClaims.find(
          (item) => item.claimId === support.target.canonicalId,
        )!;
        expect(support.paths[0]!.selectedContentDigest).toBe(
          sha256Digest(original.statement),
        );
      }
    }

    it(
      "T09 separates announcement, preview, API GA and later ChatGPT GA through entity filters",
      async () => {
        const apiEntity = claim("api-ga").entityId,
          chatgptEntity = claim("chatgpt-ga").entityId;
        expect(apiEntity).not.toBe(chatgptEntity);
        expectMembers(await scopedPacket({ at: "2026-01-16T00:00:00Z" }), [
          "api-announced",
        ]);
        expectMembers(
          await scopedPacket({
        ...
        expectMembers(await scopedPacket({ at: "2026-06-15T00:00:00Z" }), [
          "api-ga",
          "chatgpt-preview",
        ]);
        expectMembers(
    ...
    it(
      "T08 returns old belief at K0 and corrected belief at K1 while preserving adjacent worlds and original citations",
      async () => {
        const original = claim("api-ga"),
          correction = claim("api-correction");
        const k0 = claim("chatgpt-ga").knowledgeSeq;
        expect(correction.knowledgeSeq).toBeGreaterThan(k0);
        const oldPacket = await scopedPacket({
        ...
        expectMembers(
          await scopedPacket({
            at: "2026-06-11T00:00:00Z",
            entityId: original.entityId,
            knowledgeSeq: correction.knowledgeSeq,
          }),
          ["api-correction"],
        );
        expectMembers(
          await scopedPacket({
            at: "2026-06-09T23:59:59.999999Z",
            entityId: original.entityId,
          }),
          ["api-preview"],
        );
        expectMembers(
          await scopedPacket({
            at: "2026-06-11T23:59:59.999999Z",
            entityId: original.entityId,
          }),
          ["api-correction"],
        );
        ...
        const replayed = await api.inject({
          method: "GET",
          url: `/v1/evidence-packets/${oldPacket.id}/citations`,
          headers: {
            authorization: `Bearer ${TOKEN_A}`,
            "x-tenant-id": fixture.tenantId,
            "x-correlation-id": `correction-replay-${oldPacket.id}`,
          },
        });
        expect(replayed.statusCode, replayed.body).toBe(200);
        ...
        expect(replay.failures).toEqual([]);
        expect(
          replay.citations.map((citation) => citation.selectedText),
        ).toEqual([original.statement]);
      },
      FIXTURE_TIMEOUT_MS,
    );

    it(
      "R03 exposes admitted challenge and canonical correction lineage only when known",
      async () => {
        const original = claim("api-ga"),
          correction = claim("api-correction"),
          challenge = claim("api-challenge");
        const corrected = await scopedPacket({
          at: "2026-06-11T00:00:00Z",
          entityId: original.entityId,
        });
        ...
        const beforeChallenge = await scopedPacket({
          at: "2026-06-15T00:00:00Z",
          entityId: original.entityId,
          knowledgeSeq: correction.knowledgeSeq,
        });
        ...
        const disputed = await scopedPacket({
          at: "2026-06-15T00:00:00Z",
          entityId: original.entityId,
          knowledgeSeq: challenge.knowledgeSeq,
        });
        ...
        for (const member of disputed.members) {
          ...
          expect(member.contradictionIds).toEqual([
            own === original.claimId ? challenge.claimId : original.claimId,
          ]);
        }
        const old = await scopedPacket({
          at: "2026-06-11T00:00:00Z",
          entityId: original.entityId,
          knowledgeSeq: original.knowledgeSeq,
        });
...
describe.skipIf(!databaseUrl || !storage)(
  "R02 canonical source-family diversity",
  () => {
    let fixture: SelectedCandidateFixture;
    let api: ReturnType<typeof buildServer>;
    ...
    beforeAll(async () => {
      fixture = await buildRetrievalHistoryFixture({
        ...
        temporalEvents: retrievalHistoryEvents.map((event, index) =>
          index === 1
            ? { ...event, sourceFamily: "independent-analyst" }
            : event,
        ),
      });
    ...
    it(
      "keeps independent captured evidence when raw material and five reviewed summaries share one source",
      async () => {
        ...
        expect(
          all.members.filter(
            (member) => member.support!.target.kind === "summary",
          ),
        ).toHaveLength(5);
        ...
        for (const member of all.members) {
          ...
          expect(
            new Set(member.support!.paths.map((path) => path.sourceFamilyId)),
          ).toEqual(new Set([family]));
        ...
        expect(
          new Set(
            diverse.members.flatMap(
              (member) => member.support!.sourceFamilyIds,
            ),
          ),
        ).toEqual(new Set(groups.keys()));
        const independent = fixture.temporalClaims.find(
          (claim) => claim.key === "ga",
        )!;
        expect(
          diverse.members.some(
            (member) =>
              member.support!.target.canonicalId === independent.claimId,
          ),
        ).toBe(true);
        expect(fixture.candidate.publishable).toBe(false);
      },
