...
const headers = {
  ...
  "x-tenant-id": tenant,
  "x-correlation-id": "audit-route",
  "idempotency-key": "audit-route-key",
};
const request = {
  verificationContractVersion: "verification.v1",
  auditBundle: { artifactId: id(4), digest: `sha256:${"a".repeat(64)}` },
};
...
describe("audit inspection HTTP routes", () => {
  it("fails before enqueue without an exact configured claims/report grant", async () => {
    for (const admission of [undefined, async () => false]) {
      const operations = new KnowledgeIntegrationService(),
        server = buildServer({
          ...base(operations),
          ...(admission ? { isAuditInspectionRequestAdmitted: admission } : {}),
        });
      try {
        ...
        expect(response.statusCode).toBe(admission ? 403 : 503);
  ...
  it("enqueues only the strict request and exposes a separate typed terminal read", async () => {
    const operations = new KnowledgeIntegrationService(),
      server = buildServer({
        ...base(operations),
    ...
    try {
      ...
      expect(operations.input(operationId, tenant)).toMatchObject({
      ...
      ).toMatchObject({ operationId, resultArtifact: { artifactId: id(6) } });
      expect(
        (
          await server.inject({
            method: "POST",
            url: "/v1/verification/audit-bundles:inspect",
            headers,
            payload: { ...request, publicKey: "caller" },
          })
        ).statusCode,
      ).toBe(400);
    } finally {
  ...
  it("rejects malformed runtime projections before they cross the public route", async () => {
    const operations = new KnowledgeIntegrationService(),
      server = buildServer({
        ...base(operations),
        verificationAuditInspectionReads: {
