...
describe("verification run HTTP read admission", () => {
  it("binds all case/evidence reads to authenticated tenant and validates paging before dispatch", async () => {
    const tenantId = randomUUID(),
      id = randomUUID(),
      artifact = (n: number) => ({
        artifactId: `00000000-0000-4000-8000-${String(n).padStart(12, "0")}`,
    ...
    try {
      for (const url of [
        ...
        expect(
          (
            await server.inject({
              url,
              headers: { ...headers, "x-tenant-id": randomUUID() },
            })
          ).statusCode,
        ).toBe(403);
      }
      expect(reads.listRunCases).not.toHaveBeenCalled();
      expect(reads.getCase).not.toHaveBeenCalled();
      expect(reads.getEvidence).not.toHaveBeenCalled();
      const cursor = randomUUID();
      expect(
        (
          await server.inject({
            url: `/v1/verification/runs/${id}/cases?pageSize=2&cursor=${cursor}`,
            headers,
          })
        ).statusCode,
      ).toBe(200);
      expect(reads.listRunCases).toHaveBeenCalledWith({
        tenantId,
        runId: id,
        pageSize: 2,
        cursor,
      ...
      for (const query of [
        ...
        expect(
          (
            await server.inject({
              url: `/v1/verification/runs/${id}/cases?${query}`,
              headers,
            })
          ).statusCode,
        ).toBe(400);
      ...
      expect(
        (
          await server.inject({
            url: `/v1/verification/evidence/${id}`,
            headers,
          })
        ).statusCode,
      ).toBe(200);
      expect(reads.getEvidence).toHaveBeenCalledWith({
        tenantId,
        evidenceId: id,
      });
    } finally {
  ...
  it("sanitizes case/evidence integrity failures and keeps missing identities distinct", async () => {
    ...
    try {
      for (const [url, status] of [
        [`/v1/verification/runs/${id}/cases`, 503],
        [`/v1/verification/cases/${id}`, 404],
        [`/v1/verification/evidence/${id}`, 503],
      ] as const) {
        const response = await server.inject({ url, headers });
  ...
  it("denies missing and foreign credentials before reading a run", async () => {
    ...
    try {
      ...
      expect(getRunManifest).not.toHaveBeenCalled();
    } finally {
      await server.close();
    }
  });
  it("distinguishes missing run from sanitized integrity failures", async () => {
    const tenantId = randomUUID(),
      runId = randomUUID();
    const missing = Object.assign(new Error("private object key"), {
      code: "NOT_FOUND",
    });
    const getRun = vi.fn().mockRejectedValue(missing),
      getRunManifest = vi.fn().mockRejectedValue(new Error("private token"));
    const server = buildServer({
      resolveIdentity: () => ({
        actor: { kind: "human", id: randomUUID() },
        grants: [{ tenantId, roles: ["knowledge_reader"], scopes: [] }],
    ...
    try {
      const missingResponse = await server.inject({
        url: `/v1/verification/runs/${runId}`,
        headers,
      });
      expect(missingResponse.statusCode).toBe(404);
      expect(missingResponse.body).not.toContain("private");
      const broken = await server.inject({
        url: `/v1/verification/runs/${runId}/manifest`,
        headers,
      });
      expect(broken.statusCode).toBe(503);
      expect(broken.body).not.toContain("private");
      expect(getRun).toHaveBeenCalledWith({ tenantId, runId });
      expect(
        (
          await server.inject({
            url: "/v1/verification/runs/not-a-run",
            headers,
          })
        ).statusCode,
      ).toBe(400);
  ...
  it("rejects malformed verification run and manifest projections before response serialization", async () => {
    const tenantId = randomUUID(),
      runId = randomUUID(),
      server = buildServer({
        resolveIdentity: () => ({
    ...
    try {
      for (const suffix of ["", "/manifest"]) {
        const response = await server.inject({
          url: `/v1/verification/runs/${runId}${suffix}`,
          headers,
        });
        expect(response.statusCode).toBe(503);
        expect(response.body).not.toContain("private-storage-coordinate");
      }
    } finally {
      await server.close();
    }
