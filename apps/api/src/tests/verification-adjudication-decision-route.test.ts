...
const headers = {
  authorization: "Bearer review-test",
  "x-tenant-id": id(1),
  "x-correlation-id": "review-test",
  "idempotency-key": "review-test-001",
};
const request = {
  verificationContractVersion: "verification.v1",
  subjectId: id(3),
  packetArtifact: { artifactId: id(4), digest: `sha256:${"a".repeat(64)}` },
  decision: "affirm",
  rationale: "Synthetic engineering test",
};
function fixture(admitted?: boolean, reviewer = true) {
  ...
  const server = buildServer({
    ...
    resolveVerificationContext: async (input) => ({
...
describe("packet-bound decision HTTP boundary", () => {
  it("does not enqueue when runtime or reviewer grant is absent", async () => {
    for (const [admitted, status] of [
      ...
      try {
        ...
        expect(f.operations.get(id(5), id(1))).toBeUndefined();
      } finally {
  ...
  it("rejects request authority injection and forwards authenticated identity to grant checking", async () => {
    ...
    try {
      ...
      expect(f.grant).toHaveBeenCalledWith({
        request,
        context: expect.objectContaining({ tenantId: id(1), actor }),
      });
      expect(f.operations.input(id(5), id(1))).toEqual({
