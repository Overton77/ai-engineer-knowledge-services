...
describe("authenticated HTTP A2A transport", () => {
  it("routes retrieval through the retrieval-only port and canonical executor exactly once",async()=>{
    const generic=new KnowledgeIntegrationService(),retrieval=new KnowledgeIntegrationService();
    const execute=vi.fn<CanonicalRetrievalExecutorPort["execute"]>(async()=>({retrievalRunId:id(30),evidencePacketId:id(34),resultCount:0,abstained:true,replayed:false}));
    const retrievalContext={...context,operationId:id(30),attemptId:id(31),idempotencyKey:"a2a-retrieval-0001"};
    const retrievalTask={...task,taskId:id(32),kind:"retrieval" as const,context:retrievalContext,capabilityVersions:{retrieval:"v1"},expectedOutputContract:"evidence-packet/v1",operationInput:{plan:{policyVersion:id(33),query:"canonical retrieval",intents:["knowledge_evidence"],subqueries:[{id:"q",text:"canonical retrieval",coverageRole:"required"}],spaces:["engineering_claims"],anchors:{entities:[],concepts:[],useCases:[]},hardFilters:[],softBoosts:[],temporalScope:{},candidateK:10,finalK:5,graph:{maxDepth:0,allowedEdges:[]},abstention:{minimumCoverage:0.5}}}};
    const api=buildServer({service:generic,operationService:generic,retrievalOperationService:retrieval,canonicalRetrievalExecutor:{execute},publicOrigin:"https://knowledge.example",resolveIdentity:(candidate)=>candidate===token?identity:undefined});
    const response=await api.inject({method:"POST",url:"/v1/a2a/tasks",headers,payload:retrievalTask});const replay=await api.inject({method:"POST",url:"/v1/a2a/tasks",headers,payload:retrievalTask});expect(response.statusCode).toBe(202);expect(replay.json().operationId).toBe(response.json().operationId);expect(generic.get(retrievalContext.operationId)).toBeUndefined();expect(retrieval.list(tenantId)).toHaveLength(1);expect(retrieval.get(retrievalContext.operationId)?.kind).toBe("retrieval_run");expect(execute).toHaveBeenCalledTimes(2);expect(execute.mock.calls[0]?.[0]).toMatchObject({input:{plan:{query:"canonical retrieval"},a2a:{taskId:retrievalTask.taskId}},expectedVersions:{retrieval:"v1"}});await api.close();
  });
  ...
  it("rejects actor, tenant, correlation, signature-reference, and lineage substitution", async () => {
    const wrongActor = await server.inject({
      ...
      url: "/v1/a2a/tasks",
      headers,
      payload: { ...task, context: { ...context, actor: { ...actor, id: id(99) } } },
    });
    ...
    await server.inject({ method: "POST", url: "/v1/a2a/tasks", headers, payload: freshTask });
    const callback = new A2AKnowledgeAdapter(
      service,
      "https://knowledge.example",
    ).callback(freshTask, { outcome: "succeeded" }, secret, clock().toISOString());
