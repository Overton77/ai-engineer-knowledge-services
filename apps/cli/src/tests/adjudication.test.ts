...
const context: OperationContext = {
  ...
  operationId: id(2),
  attemptId: id(3),
  correlationId: "adjudication-cli",
  actor: {
    kind: "service",
    id: id(4),
  ...
  idempotencyKey: "adjudication-cli-001",
  reason: "test",
  contractVersion: "v1",
};
const request = {
  ...
  reason: "appeal",
  evidencePacket: { artifactId: id(5), digest: `sha256:${"a".repeat(64)}` },
};
...
describe("adjudication CLI", () => {
  ...
  it("forwards a strict packet-bound decision without caller authority fields", async () => {
    ...
    const decision = {
      verificationContractVersion: "verification.v1",
      subjectId: id(5),
      packetArtifact: { artifactId: id(6), digest: `sha256:${"b".repeat(64)}` },
      decision: "affirm",
  ...
  it("reads a decision through the compact tenant-scoped client method", async () => {
    const getAdjudicationDecision = vi.fn(async () => ({
      output: { decision: "affirm" },
    }));
    ...
    await expect(
      dispatchCliCommand(
        client,
        resolveCommand("adjudication", "get-decision")!,
        { operationId: id(5), reviewerRole: "caller" },
        context,
      ),
    ).rejects.toThrow();
  });
