...
const context: OperationContext = {
  ...
  idempotencyKey: "benchmark-cli",
  reason: "test",
  contractVersion: "v1",
};
const request = {
  verificationContractVersion: "verification.v1",
  dataset: { artifactId: id(6), digest: `sha256:${"1".repeat(64)}` },
...
describe("benchmark CLI", () => {
  it("submits comparison references and reports an engineering gate failure without fabricating quality", async () => {
    const compareBenchmarkRuns = vi.fn(),
      request = {
        verificationContractVersion: "verification.v1",
        baselineRunId: id(20),
        candidateRunId: id(21),
        comparisonProfile: "regression_gate",
      };
    await dispatchCliCommand(
      { compareBenchmarkRuns } as unknown as CliKnowledgeClient,
      resolveCommand("benchmark", "compare")!,
      request,
      context,
    );
  ...
  it("reports completed execution without inventing a quality pass", async () => {
    ...
    const client = {
      ...
      getReceipt: vi.fn(async () => ({
        operationId: context.operationId,
        receiptKind: "replay_recorded_and_register.succeeded",
        outcome: "succeeded",
        body: { output },
      })),
    };
...
describe("claims/report --wait completion", () => {
  const manifestArtifact = {
    ...
    byteLength: 12,
    objectKey: "tenants/x/manifest.json",
    createdAt: "2026-09-08T00:00:00.000Z",
    producerActivityId: "verify_claims_and_register",
    producerVersion: "1.0.0",
    encryptionClass: "tenant-default",
    retentionClass: "standard",
    dataClassification: "internal",
    parentArtifactIds: [],
  };
  const sealedRun = (policyOutcome: string) => ({
    runId: id(31),
    ...
    getReceipt: vi.fn(async () => ({
      ...
      receiptKind:
        kind === "verification_claims"
          ? "verify_claims_and_register.succeeded"
          : "verify_report_and_register.succeeded",
      outcome: "succeeded",
      body: {
        output: {
          verified: {
          ...
          sealedRun: sealedRun(policyOutcome),
        },
      },
    })),
  });
  it("exits 0 only when mechanics passed and policy admitted", async () => {
    const admitted = await waitForVerification(
      clientFor("verification_claims", "passed", "pass_with_warnings") as never,
      context.operationId,
      context,
      1000,
    );
    expect(admitted).toMatchObject({
      exitCode: 0,
      claims: {
        policyOutcome: "pass_with_warnings",
  ...
  it("holds review/abstain outcomes and fails failed outcomes with exit 1", async () => {
    const held = await waitForVerification(
      clientFor("verification_report", "review_required", "review") as never,
      context.operationId,
      context,
      1000,
    );
    expect(held).toMatchObject({
      exitCode: 1,
      report: { disposition: "held_for_review", policyOutcome: "review" },
    });
    const failed = await waitForVerification(
      clientFor("verification_report", "failed", "fail") as never,
      context.operationId,
      context,
      1000,
    );
    expect(failed).toMatchObject({
      exitCode: 1,
      report: { disposition: "quality_failed" },
    });
    const policyFailedDespitePass = await waitForVerification(
      clientFor("verification_claims", "passed", "fail") as never,
      context.operationId,
      context,
      1000,
    );
    expect(policyFailedDespitePass).toMatchObject({
      exitCode: 1,
      claims: { disposition: "quality_failed" },
    });
  });
  ...
  it("rejects malformed sealed results instead of guessing", async () => {
    ...
    await expect(
      waitForVerification(
        clientFor("verification_claims", "unknown", "pass") as never,
        context.operationId,
        context,
        1000,
      ),
    ).rejects.toThrow("VERIFICATION_RESULT_INVALID");
  });
});
