...
const client = () =>
  ({
    ...
    validateRetrievalPlan: vi.fn(),
    createVectorStore: vi.fn(),
    attachVectorStoreDocuments: vi.fn(),
    getVectorStore: vi.fn(),
    getVectorStoreOperation: vi.fn(),
    getOperation: vi.fn(),
    getVerificationOperation: vi.fn(),
    getOperationEvents: vi.fn(),
    retryOperation: vi.fn(),
    reconcileOperation: vi.fn(),
    getRetrievalExplanation: vi.fn(),
    getEvaluationFailures: vi.fn(),
    getAuditInspection: vi.fn(),
    getVerificationClaimsResult: vi.fn(),
    getVerificationReportResult: vi.fn(),
    inspectAuditBundle: vi.fn(),
    captureVerificationSource: vi.fn(),
    parseArtifact: vi.fn(),
    verifyExtraction: vi.fn(),
    verifyClaims: vi.fn(),
    verifyReport: vi.fn(),
    replayVerificationRun: vi.fn(),
...
describe("CLI semantic dispatcher", () => {
  it("routes store reads using real ids", async () => {
    ...
    await dispatchCliCommand(
      ...
      { vectorStoreId: id(6), operationId: id(7) },
      context,
    );
  ...
  it.each([
    ["promotion", "status", "getOperation", "operationId"],
    ["embed", "status", "getOperation", "operationId"],
    ["retrieve", "explain", "getRetrievalExplanation", "runId"],
    ...
    const c = client(),
      input = {
        ...
        documents: [
          {
            documentId: id(7),
            documentVersionId: id(8),
            representationId: id(9),
            requestedProfile: { profile: "canonical" },
          },
        ],
      };
    await dispatchCliCommand(
  ...
  it("routes parseArtifact through the strict client method", async () => {
    const c = client(),
      input = {
        verificationContractVersion: "verification.v1",
        captureId: id(10),
  ...
  it("routes claim and report verification through strict client methods", async () => {
    const c = client(),
      ref = { artifactId: id(10), digest: `sha256:${"1".repeat(64)}` },
      claims = {
        verificationContractVersion: "verification.v1",
        captureIds: ["capture-1"],
        assertions: ref,
  ...
  it("routes strict audit inspection submission and typed terminal reads", async () => {
    const c = client(),
      request = {
        verificationContractVersion: "verification.v1",
...
it("routes claims and report terminal reads without accepting extra fields", async () => {
  ...
  await expect(
    dispatchCliCommand(
      c,
      resolveCommand("verify", "claims-result")!,
      { operationId: id(14), rawBundle: true },
      context,
    ),
  ).rejects.toThrow("VERIFICATION_RESULT_READ_INPUT_INVALID");
});
