...
import {
  isVerificationAttestationCliError,
...
const group = args[0] ?? "",
  action = args[1] ?? "",
  command = resolveCommand(group, action);
const print = (value: unknown) =>
  process.stdout.write(
    `${JSON.stringify(value, human ? null : undefined, human ? 2 : undefined)}\n`,
  );
if (group === "demo" && action === "diagnostics-companies") {
  try {
    ...
    process.stderr.write(
      `${JSON.stringify({ code: "DEMO_ERROR", message: error instanceof Error ? error.message : "Unknown error" })}\n`,
    );
    process.exitCode = 2;
  }
} else if (
  group === "benchmark" &&
  action === "capture" &&
  args[2] === "diagnostics-companies"
) {
  try {
    const result = await runDiagnosticsBenchmarkCapture(args, {
      writer: { writeBenchmarkRefreshProposal },
    });
    print(result);
    process.exitCode = result.exitCode;
  } catch (error) {
    process.stderr.write(
      `${JSON.stringify({ code: "BENCHMARK_CAPTURE_ERROR", message: error instanceof Error ? error.message : "Unknown error" })}\n`,
    );
    process.exitCode = 2;
  }
} else if (group === "benchmark" && action === "diff") {
  try {
    print(await runLocalBenchmarkVersionDiff(args));
  } catch (error) {
    process.stderr.write(
      `${JSON.stringify({ code: "BENCHMARK_DIFF_ERROR", message: error instanceof Error ? error.message : "Unknown error" })}\n`,
    );
    process.exitCode = 2;
  }
} else if (group === "verification" && action === "attestation-export") {
  try {
    const result = await runVerificationAttestationExport(args);
    print(result.output);
    process.exitCode = result.exitCode;
  } catch (error) {
    process.stderr.write(
      `${JSON.stringify({ code: isVerificationAttestationCliError(error) ? error.code : "ATTESTATION_EXPORT_FAILED" })}\n`,
    );
    process.exitCode = 2;
  }
} else if (group === "verification" && action === "attestation-inspect") {
  try {
    const result = await runVerificationAttestationInspect(args);
    print(result.output);
    process.exitCode = result.exitCode;
  } catch (error) {
    process.stderr.write(
      `${JSON.stringify({ code: isVerificationAttestationCliError(error) ? error.code : "ATTESTATION_INSPECTION_FAILED" })}\n`,
    );
    process.exitCode = 2;
  }
} else if (!command) {
  process.stderr.write(
    `${JSON.stringify({ code: "UNKNOWN_COMMAND", group, action })}\n`,
  );
  process.exitCode = 2;
} else
  try {
    ...
    const baseUrl = option("--base-url") ?? process.env.KNOWLEDGE_API_URL;
    if (!baseUrl)
      throw new Error("KNOWLEDGE_API_URL or --base-url is required");
    const token = process.env.KNOWLEDGE_API_TOKEN;
    if (!token) throw new Error("KNOWLEDGE_API_TOKEN is required");
    const context = OperationContextSchema.parse(
      JSON.parse(option("--context") ?? "null"),
    );
    ...
    if (!Number.isInteger(timeoutMs) || timeoutMs < 100 || timeoutMs > 300000)
      throw new Error("CLI_TIMEOUT_INVALID");
    ...
    process.stderr.write(
      `${JSON.stringify({ code: "CLI_ERROR", message: error instanceof Error ? error.message : "Unknown error" })}\n`,
    );
    process.exitCode = 2;
  }
