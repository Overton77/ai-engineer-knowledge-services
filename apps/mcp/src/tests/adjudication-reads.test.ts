...
describe("adjudication MCP reads", () => {
  it("rejects tenant and authority injection before dispatch and forwards only exact read identifiers", async () => {
    ...
    const getAdjudicationSubject = vi
      .fn()
      .mockResolvedValue({ output: { status: "pending_human_adjudication" } });
    const options = {
      ...
      apiClient: { getAdjudicationSubject } as never,
    };
    ...
    getAdjudicationSubject.mockRejectedValueOnce(
    ...
    await expect(
      createAdjudicationReadMcpExecutor({ ...options, apiClient: undefined })({
        context,
        operationId,
      }),
    ).resolves.toMatchObject({ isError: true });
  ...
  it("reads a decision with exact compact identifiers and rejects caller authority injection", async () => {
    ...
    const getAdjudicationDecision = vi
      .fn()
      .mockResolvedValue({ output: { decision: "affirm" } });
    const options = {
      operationService: {} as never,
      apiOrigin: "https://knowledge.example",
      identity: {
      ...
      apiClient: { getAdjudicationDecision } as never,
    };
