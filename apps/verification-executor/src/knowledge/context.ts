...
import { IngestionExecutor, ReportService, REPORT_BUCKET } from "@aiengineer/knowledge-ingestion";
import { TenantPostgres, PostgresCanonicalRepository, PostgresSourceDiscoveryStore, PostgresCheckpointStore, PostgresGovernedIndexRepository } from "@aiengineer/knowledge-persistence";
...
export function createKnowledgeServices(config: KnowledgeConfig, options: KnowledgeServicesOptions = {}): KnowledgeServices {
  ...
  const artifacts = new ArtifactLedger({ db, store, bucket: LEDGER_BUCKET, uploaded, executorVersion: KNOWLEDGE_EXECUTOR_VERSION, readStores: { [REPORT_BUCKET]: reportStore } });
  ...
  return { config, db, workspace, artifacts, reads, ingestion, reports,
    ...(sourceDiscovery ? { sourceDiscovery } : {}),
    ...(checkpoints ? { checkpoints } : {}), ...(checkpointHarness ? { checkpointHarness } : {}),
    close: async () => { await discoveryDatabase?.close(); await custody?.close(); await db.close(); },
  };
}
