...
export async function resolveSafeHttpTarget(
  ...
  if (
    policy.deniedHosts?.includes(host) ||
    host === "localhost" ||
...
export function buildPinnedRequestOptions(
  ...
  return {
    ...
    ...(url.protocol === "https:"
      ? {
          ...(isIP(certificateName) === 0
            ? { servername: certificateName }
            : {}),
...
async function boundedBody(
  ...
  try {
    ...
    while (true) {
      ...
      if (encoded && size / declared > maximumDecompressionRatio)
...
export class ExactHttpAcquisitionAdapter implements AcquisitionAdapter {
  ...
  async plan(request: AcquisitionRequest): Promise<AcquisitionPlan> {
    ...
    const digestablePolicy = JSON.parse(
      JSON.stringify({
        ...this.policy,
        allowedProtocols: [...this.policy.allowedProtocols],
        allowedPorts: [...this.policy.allowedPorts],
      }),
    );
    return {
      ...
      policyDigest: sha256Digest(digestablePolicy),
    };
  }
  async execute(plan: AdmittedAcquisitionPlan): Promise<AcquisitionResult> {
    ...
    try {
      return await Promise.race([
        this.executeWithinDeadline(plan, controller.signal),
        deadline,
      ]);
    } finally {
      clearTimeout(timer);
  ...
  private async executeWithinDeadline(
    ...
    for (let count = 0; count <= this.policy.maximumRedirects; count++) {
      ...
      const init: RequestInit = {
        ...
        signal,
        headers: {
          accept: plan.request.preferredMediaTypes.join(", ") || "*/*",
          "accept-encoding": "identity",
          "user-agent": "ai-engineer-knowledge-services/1",
        },
      };
      response = this.fetcher
        ? await this.fetcher(target.url.href, init)
        : await this.pinnedTransport.fetch(target.url, init, target.addresses);
      if (signal.aborted) {
    ...
    const bytes = await boundedBody(
      response,
      Math.min(plan.request.maximumBytes, this.policy.maximumBytes),
      this.policy.maximumDecompressionRatio,
      signal,
    );
    signal.throwIfAborted();
    const mediaType =
    ...
    const redactedHeaders = [...response.headers.entries()]
      .filter(
        ([key]) => !/authorization|cookie|api[-_]?key|secret|token/i.test(key),
      )
    ...
    return {
      ...
      observations: [
        ...
        { key: "final_url", value: url },
        { key: "redirects", value: JSON.stringify(redirects) },
        { key: "headers", value: JSON.stringify(redactedHeaders) },
        {
