import { type VectorSearchBackend, type VectorSearchCandidate, type VectorSearchRequest, VectorBackendError, validateEmbedding, validateSearchLimit } from "./types.js";

export interface PostgresRpcResult {
  readonly data: unknown;
  readonly error: null | { readonly message?: string; readonly code?: string; readonly details?: string };
}
...
export class PostgresVectorSearchBackend implements VectorSearchBackend {
  ...
  async search(request: VectorSearchRequest): Promise<readonly VectorSearchCandidate[]> {
    ...
    if (result.error !== null) {
      throw new VectorBackendError(result.error.code ?? "POSTGRES_RPC_FAILED", result.error.message ?? "Postgres vector search failed", result.error);
    }
    if (!Array.isArray(result.data)) throw new VectorBackendError("INVALID_RPC_RESPONSE", "Postgres vector search returned a non-array response");
...
function parseCandidate(value: unknown, index: number): VectorSearchCandidate {
  ...
  if (typeof row.vector_item_id !== "string" || row.vector_item_id.length === 0 || typeof row.score !== "number" || !Number.isFinite(row.score)) {
    throw new VectorBackendError("INVALID_RPC_RESPONSE", `Candidate ${index} has invalid identity or score`);
  }
  if (row.search_projection_id !== null && row.search_projection_id !== undefined && typeof row.search_projection_id !== "string") {
