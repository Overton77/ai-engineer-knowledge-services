...
import { VerificationClaimsApplicationService, type ReportVerificationResult, type VerificationClaimsServiceDependencies } from "../verification/operations/verification-claims.js";
import { replayCapturedSemanticAssessment } from "../verification/operations/verification-semantic-replay.js";
...
export async function prepareDiagnosticsGeneratedReportSemantics(input: {
  ...
  for (const descriptor of input.reports) {
    ...
    for (const [ordinal, diagnostic] of cases.entries()) {
      ...
      const inputArtifactDigest = digestCanonicalJson(blindedInput), requestWire = prepareGatewaySemanticRequest({ ...blindedInput, inputArtifactDigest }, identity.model, 2_000, identity.promptDigest);
