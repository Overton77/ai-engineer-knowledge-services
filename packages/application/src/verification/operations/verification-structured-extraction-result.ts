import {
  ...
  VerificationStructuredExtractionResultSchema,
...
/**
 * Derives the compact terminal payload from an already retained, signed
 * structured-extraction publication. This is an authenticated pure helper;
 * the caller remains responsible for canonical operation-step admission.
 */
export async function createStructuredExtractionOperationResult(input: {
...
}): Promise<VerificationStructuredExtractionResult> {
  // Snapshot all caller-owned state before the only asynchronous boundary.
  const manifest = deepFreeze(VerificationStructuredExtractionPublicationSchema.parse(structuredClone(input.manifest)));
  ...
  if (structuredExtractionProviderCallDigest(manifest.providerCall) !== manifest.providerCallDigest) {
    throw new Error("STRUCTURED_EXTRACTION_RESULT_PROVIDER_CALL_INVALID");
  }
