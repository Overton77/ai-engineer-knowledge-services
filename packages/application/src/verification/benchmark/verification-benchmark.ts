import { executeDiagnosticsCompanyFieldPlan } from "../../diagnostics/verification-diagnostics-company-fields.js";
import { renderDiagnosticsCompanyFields } from "../../diagnostics/verification-diagnostics-field-view.js";
import { composeDiagnosticsPolicyReplay } from "../../diagnostics/verification-diagnostics-policy-composition.js";
import { buildDiagnosticsMutationReport } from "../../diagnostics/verification-diagnostics-mutation-report.js";
import { buildDiagnosticsOfflineLedgers } from "../../diagnostics/verification-diagnostics-offline-ledgers.js";
import { verifyDiagnosticsOfflineClaimsReportClosure } from "../../diagnostics/verification-diagnostics-offline-claims-report.js";
import { runDiagnosticsEngineeringMutations, type DiagnosticsEngineeringMutation } from "../../diagnostics/verification-diagnostics-engineering-mutations.js";
import { buildDiagnosticsReportCoverage, auditDiagnosticsReportCoverage, type DiagnosticsReportBlockInput, type DiagnosticsReportEvidenceResolution } from "../../diagnostics/verification-diagnostics-report-coverage.js";
import { replayDiagnosticsSemanticFixture } from "../../diagnostics/verification-diagnostics-semantic-replay.js";
import { loadDiagnosticsGeneratedReportSemanticFixture, replayDiagnosticsGeneratedReportSemanticFixture } from "../../diagnostics/verification-diagnostics-generated-report-semantic-fixture.js";
import { prepareDiagnosticsReportSemanticReplayComposition } from "../../diagnostics/verification-diagnostics-report-semantic-composition.js";
import { verifyDiagnosticsAdversarialProjection, type DiagnosticsAdversarialObservation } from "../../diagnostics/verification-diagnostics-adversarial.js";
import { evaluateDiagnosticsFullDemoQualityGate } from "../../diagnostics/verification-diagnostics-quality-gates.js";
import { loadDiagnosticsOfflineCatalog, type DiagnosticsOfflineCatalogName } from "../../diagnostics/verification-diagnostics-offline-catalog.js";
...
export function assertDiagnosticsProviderWireRequest(testCase: VerificationBenchmarkCase, authority: DiagnosticsProviderGrantAuthority, requestBytes: Uint8Array, policy: { readonly provider: "gateway" | "interfaze"; readonly model: "openai/gpt-5.6-luna" | "anthropic/claude-haiku-4.5" | "interfaze-beta" }): void {
  ...
  if (policy.provider === "gateway" && (policy.model === "anthropic/claude-haiku-4.5" ? providerDigest(messages[0]!.content) !== gatewaySemanticPromptDigest : messages[0]!.content !== extractionSystemPrompt)) throw new Error("BENCHMARK_PROVIDER_WIRE_SYSTEM_PROMPT_MISMATCH");
...
export function assertDiagnosticsExtractionWireRequest(testCase: VerificationBenchmarkCase, authority: DiagnosticsExtractionAuthority, requestBytes: Uint8Array, policy: { readonly provider: "gateway" | "interfaze"; readonly model: "openai/gpt-5.6-luna" | "anthropic/claude-haiku-4.5" | "interfaze-beta" }): void {
  ...
  if (policy.provider === "interfaze" ? messages.length !== 1 : messages.length !== 2 || messages[0]!.role !== "system" || (policy.provider === "gateway" && (semantic ? providerDigest(messages[0]!.content) !== gatewaySemanticPromptDigest : messages[0]!.content !== extractionSystemPrompt))) throw new Error("BENCHMARK_EXTRACTION_WIRE_SYSTEM_PROFILE_MISMATCH");
...
export async function runDiagnosticsCompaniesDemo(input: { readonly catalogDirectory: string; readonly catalogName?: DiagnosticsOfflineCatalogName; readonly semanticFixture?: { readonly directory: string; readonly expectedFixtureDigest: Digest }; readonly generatedReportSemanticFixture?: { readonly directory: string; readonly expectedFixtureDigest: Digest }; readonly sourcePreparationDirectory: string; readonly outputDirectory: string; readonly runId: string; readonly now: () => string }): Promise<DiagnosticsDemoResult> {
  ...
  const generatedReportSemanticReplay = generatedReportFixture ? await replayDiagnosticsGeneratedReportSemanticFixture({ fixture: generatedReportFixture, prepared: await prepareDiagnosticsReportSemanticReplayComposition({ dataset, judgeIdentity: generatedReportFixture.judgeIdentity, sourceRunManifestDigest: generatedReportFixture.sourceRunManifestDigest, sourceLedger, captures: preparationManifest.captures, reports: reportCoverage.reports, nativeLedgers, nativeVerification, admission }) }) : undefined;
