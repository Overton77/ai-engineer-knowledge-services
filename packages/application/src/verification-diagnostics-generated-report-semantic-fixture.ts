...
import { canonicalizeJson, GatewaySemanticJudgeAdapter, interpretCapturedGatewaySemanticResponse, prepareGatewaySemanticRequest, sha256Digest, type GatewaySemanticResponseObservation, type ProviderArtifactSink } from "@aiengineer/knowledge-verification";
...
export async function replayDiagnosticsGeneratedReportSemanticFixture(input:{readonly prepared:PreparedDiagnosticsGeneratedReportSemantics;readonly fixture:DiagnosticsGeneratedReportSemanticReplayFixture}){
 ...
 for(const [index,entry] of plan.entries.entries()){
  ...
  const wire=prepareGatewaySemanticRequest({...entry.blindedInput,inputArtifactDigest:entry.inputArtifactDigest},plan.judgeIdentity.model,plan.limits.maximumInputCharacters);
 ...
 const result=await executeDiagnosticsGeneratedReportSemantics({prepared:input.prepared,grant,beforeDispatch:async entry=>{if(!captures.has(entry.entryDigest))throw new Error("DIAGNOSTICS_REPORT_SEMANTIC_FIXTURE_REPLAY_MISSING");},async createAdapter(entry){const capture=captures.get(entry.entryDigest)!;
  const sink:ProviderArtifactSink={async assertExternalProcessingAdmission(value){if(value.providerId!=="gateway"||value.modality!=="text")throw new Error("DIAGNOSTICS_REPORT_SEMANTIC_FIXTURE_REPLAY_PROVIDER");},async persistBeforeDispatch(value){if(value.requestDigest!==entry.expectedRequestDigest||!Buffer.from(value.requestBytes).equals(Buffer.from(capture.requestBytes)))throw new Error("DIAGNOSTICS_REPORT_SEMANTIC_FIXTURE_REPLAY_REQUEST");},async persistAfterResponse(value){if(value.requestDigest!==entry.expectedRequestDigest||value.httpStatus!==capture.httpStatus||!Buffer.from(value.rawResponseBytes).equals(Buffer.from(capture.rawBytes)))throw new Error("DIAGNOSTICS_REPORT_SEMANTIC_FIXTURE_REPLAY_RESPONSE");}};
  const adapter=new GatewaySemanticJudgeAdapter({apiKey:"offline-fixture-replay",model:"openai/gpt-5.6-luna",identity:plan.judgeIdentity,maximumInputCharacters:plan.limits.maximumInputCharacters,artifactSink:sink,fetch:async()=>new Response(capture.rawBytes,{status:capture.httpStatus}),async recordObservation(value){if(!same(value,capture.observation))throw new Error("DIAGNOSTICS_REPORT_SEMANTIC_FIXTURE_REPLAY_OBSERVATION");}});return adapter;},deadlineEpochMs:()=>Date.now()+60_000,afterAssessment:async(entry,value)=>{const expected=captures.get(entry.entryDigest)?.record.assessment;if(!expected||value.failure||!same(value.assessment,expected))throw new Error("DIAGNOSTICS_REPORT_SEMANTIC_FIXTURE_REPLAY_ASSESSMENT");}});
